<template>
	<!--组件-->
	<scroll-view scroll-x='false' :scroll-into-view=" 'tab-' + swiperTabIdx" :style="{ height:currentSwiperHeight + 'rpx' }">
		<view class="swiperTab" v-for="(item,index) in swiperTabList" :key='index' 
			...
			:id=" 'tab-' + index " @click="CurrentTab(index,item)">
...
<script>
	export default{
		...
		props:{
			...
			currentSwiperLineAnimatie:{ type:Boolean },////当前选中的导航栏线条过渡效果
		},
		methods:{
